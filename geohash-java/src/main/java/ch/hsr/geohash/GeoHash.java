...
public final class GeoHash implements Comparable<GeoHash>, Serializable {
	 ...
	 *            base32精度
	 * @return 生成的{@link GeoHash}类
	 * @description 该方法使用numberOfCharacters作为所需精度值。因为哈希值最长只有64bit,
	 *              所以numberOfCharacters最大值为12
	 */
	 ...
	 *            二进制精度
	 * @return 生成的{@link GeoHash}类
	 * @description 根据numberOfBits作为精度值,创建一个新的{@link GeoHash}。与此同时设置了边界框
	 */
	...
	/**
	 * @param geohash
	 * @return 根据base32编码{@link String}创建一个新的{@link GeoHash}
	 * @description 该方法同时设置了哈希值、边界框等值，因此也可以和其他方法连用，例如within()
	 */
	...
	private static void setBoundingBox(GeoHash hash, double[] latitudeRange, double[] longitudeRange) {
		hash.boundingBox = new BoundingBox(new WGS84Point(latitudeRange[0], longitudeRange[0]),
	...
	/**
	 * @return 该{@link GeoHash}的base32编码精度
	 ...
	 *            GeoHash
	 * @return 间隔数
	 * @description 计算两个{@link GeoHash}间隔的{@link GeoHash}
	 *              数量(即运行几次next()从GeoHash1到GeoHash2)。该值取决于有效位
	 */
	...
	/**
	 * @return 8个相邻的边界框
	 * @description 固定排序为顺时针：N, NE, E, SE, S, SW, W, NW
	 */
	...
	/**
	 * @return 获取{@link GeoHash}的base32编码
	 * @throws IllegalStateException
	 *             when the number of significant bits is not a multiple of 5.
	 * @description 只有当significant有效位为5的倍数，该方法才有效
	 */
	 ...
	 * @param point
	 *            坐标
	 * @return 判断该坐标是否位于该{@link GeoHash}的边界框内
	 * @description <i>注意: 该方法仅判断{@link GeoHash}对应的边界框是否包含该坐标。因此并没有使用到
	 *              {@link GeoHash}的特性</i>
	 */
	...
	/**
	 * @return 返回当初创建该{@link GeoHash}的{@link WGS84Point}
	 * @description 如果当初使用base32编码{@link String}创建该{@link GeoHash}，则返回对应边界框的中心点
	 */
	...
	/**
	 * @return 返回{@link GeoHash}对应边界框的中心点。该值不保证与创建该{@link GeoHash}所使用的坐标相同！
	 */
